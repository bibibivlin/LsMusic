...
Keep DLNA networking isolated from internet traffic. Never bind the application process to Wi-Fi with `ConnectivityManager.bindProcessToNetwork`, because this also redirects ListenBrainz, artwork, and other HTTPS/DNS traffic and can leave them pinned to a stale network. If DLNA requires an explicit interface, bind only its sockets or individual connections. Internet requests must continue to follow Android's default network and active VPN.

Release builds must preserve the jUPnP control and XML pipeline from R8 optimization and renaming. Keep the `-keep,allowshrinking class org.jupnp.** { *; }` rule in `app/src/main/keepRules/rules.keep`: `allowshrinking` is required so unused desktop and OSGi adapters are not packaged, while the remaining protection prevents release-only Browse SOAP serialization failures. After changing jUPnP dependencies, R8 configuration, or release optimization, build `:app:assembleRelease` and verify media-library browsing on a physical device; a successful debug browse is not sufficient.
...
Cold-start device restoration must not expose SSDP discovery order. Load the remembered media-server and renderer IDs plus their cached display metadata before the first visible frame, keep those selections pinned while discovery is running, and never temporarily fall back to the first discovered server or the local renderer. Preserve a manual selection made during discovery. Treat waiting for the remembered server and the initial root browse as one continuous library-loading phase, and enter the browse-loading state in the same state update that makes the server available so no transient empty-directory frame can render. Show an empty directory only after a successful empty browse result; after discovery completes, report an unavailable remembered device explicitly instead of silently switching devices.
...
Keep media-library search and fast scrolling available in both list and grid layouts. Once the in-content search field has fully scrolled away, show a circular search control matching the field height; activating it must return to and focus the original field. Show the right-edge fast scroller only for overflowing content, give its narrow visual thumb a sufficiently wide touch target and adjustable accessibility semantics, and keep it clear of the floating search control and mini player. Fast scrolling must operate through the existing lazy state without preloading the directory or breaking saved-position restoration.

Keep the mini player as an overlay above page content rather than part of `Scaffold.bottomBar`, so scrolling content can pass behind it. Scrollable destinations must reserve enough trailing content padding for their final item to scroll fully above the player, while the player itself leaves enough space above bottom navigation for its shadow to finish without clipping. The now-playing seek control must retain slider seeking and accessibility behavior: animate only its played segment as a wave while playback is active, and return it to a straight line when playback is paused or stopped.
...
ListenBrainz metadata enrichment must preserve MusicBrainz entity semantics: recording, release, release group, release-track, and artist MBIDs are distinct and must never be substituted for one another. Prefer the recording artist for `artist_name` and artist MBIDs; a composer must not become the reported artist, and album-artist IDs must not replace track-artist IDs. Enrich reporting metadata from embedded MP3, FLAC, DSF, and DFF tags on a field-by-field basis, with embedded values taking precedence over DIDL and failures falling back safely to the original DIDL metadata. Remote tag inspection must use bounded HTTP range reads, honor container-declared file lengths, and never download the complete audio resource. Apply the same enriched metadata to `playing_now` and permanent listens.
...
Use JUnit4 for deterministic logic tests. Name tests as behavior statements, for example `parseTimeMs_supportsDlnaFractionalDuration`. Add unit tests for parsing, queue behavior (including initial play-all selection, repeat boundaries, shuffle exhaustion, shuffle reset, and tracks added during shuffle), album ordering (including both year directions, missing metadata, multilingual title groups, and pinyin), remembered-device restoration during discovery, initial library-loading state transitions, browse-cache hits and empty directories, stale-request rejection, LRU eviction, directional prefetch ranges, fast-scroll position and target clamping, playback/reporting state transitions, threshold evaluation, MusicBrainz metadata extraction, and token-validation error classification. Cover embedded-tag parsing with deterministic binary fixtures, including remote range boundaries, end-of-file handling, malformed tags, and fallback when optional prefetching times out. Unit tests must not call the live ListenBrainz service. Reserve `androidTest` for Android framework or UI integration. No coverage threshold is configured—cover changed behavior proportionately. For network-routing changes, verify ListenBrainz on a mobile connection with the system VPN active, then switch between Wi-Fi and mobile/VPN without restarting the app; confirm DLNA discovery and control still work after returning to Wi-Fi. Validate media-library scrolling and artwork behavior on a physical device with a library containing hundreds of albums; include cold-cache, warm-cache, fast-fling, right-edge dragging in list and grid modes, floating-search return, album-entry, and back-navigation cases, preferably with an optimized release build.

## Commit & Pull Request Guidelines

The current history only contains `Initial commit`, so no established commit format exists. Use concise imperative subjects, preferably scoped, such as `dlna: add SOAP compatibility fallback`. Keep commits focused.
