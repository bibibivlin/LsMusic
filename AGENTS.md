...
- `ui/`: Compose screens, `LsMusicViewModel`, preferences, theme files, and client-side album sorting in `AlbumSorting.kt`.
...
Use JUnit4 for deterministic logic tests. Name tests as behavior statements, for example `parseTimeMs_supportsDlnaFractionalDuration`. Add unit tests for parsing, queue behavior, album ordering (including both year directions, missing metadata, multilingual title groups, and pinyin), playback/reporting state transitions, threshold evaluation, MusicBrainz metadata extraction, and token-validation error classification. Unit tests must not call the live ListenBrainz service. Reserve `androidTest` for Android framework or UI integration. No coverage threshold is configured—cover changed behavior proportionately.
...
Pull requests should explain user-visible behavior, identify affected playback paths (local or remote), list verification commands, and include screenshots for UI changes. For media-library sorting changes, include a narrow-phone screenshot and describe server-order preservation plus missing-metadata behavior. For DLNA fixes, record the tested server/renderer family and any compatibility fallback. For ListenBrainz changes, describe `playing_now` and permanent-listen behavior, threshold semantics, metadata fallback, token validation, and whether a test account was used.
