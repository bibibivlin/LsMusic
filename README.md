...
设置首页保留完整的媒体库与播放设备选择区域；“界面”“播放”“歌词”和“网络”分别通过滑动与淡入过渡进入独立页面。“关于”中可查看软件版本、项目主页、隐私说明和开源声明。

## 播放设置

在“设置 > 播放”中可以调整以下行为，本机播放与 DLNA 播放使用相同规则：

- **用加入队列代替正在播放时的点击播放操作**：默认关闭。开启后，音乐播放中点击媒体库歌曲、播放全部或随机播放，只会将所选曲目加入队尾，不打断当前歌曲；暂停时不生效。
- **迷你播放器**：默认开启。关闭后，其他页面不再显示迷你播放器，仍可从底部导航进入“正在播放”进行控制。
- **开始播放时清空当前播放列表**：默认开启。关闭后，点击媒体库歌曲或播放按钮会将所选曲目追加到队尾，并从本次新增的第一首开始播放；上一项的“仅加入队列”规则优先。

队列允许重复歌曲，每次添加都保留为独立的一项。队列内点歌始终直接切换到该项，暂停／继续、上下曲和系统媒体控制不受上述媒体库点击规则影响。

同一页面还提供**睡眠定时器**：可选择 15、30、45、60、90 分钟或自定义 1～180 分钟，查看剩余时间、重新计时或取消。应用会记住上次时长和模式，每次仍需手动启动。首次使用需要在系统设置中允许“闹钟和提醒”，授权后才开始计时。暂停音乐、切歌、更换播放器、后台运行和锁屏均不暂停倒计时。

默认到点立即暂停并保留进度；开启“到点后播完当前曲目再停止”后，会等待到点时的歌曲结束，即使开启循环也不会继续播放。等待期间手动暂停或停止会完成定时，手动切歌或更换播放器会取消等待；歌曲结束后再次播放从该曲开头开始。运行中的定时器不在主动退出、进程终止或设备重启后恢复。DLNA 设备若不支持暂停，会尝试停止并保留队列；设备断连或无法确认结果时会显示失败提示。
...
播放与定时的真机测试使用独立测试应用，避免触碰已有账户和播放记录：`./gradlew -I gradle/playback-qa.init.gradle :app:connectedDebugAndroidTest`。连接多台设备时先用 `ANDROID_SERIAL` 指定目标。测试完成后重新运行 `:app:assembleDebug`，生成正常包名的安装包。
